<template>
	<div>
		<Button label="Print" icon="pi pi-print" @click="printClick" style="display: block; margin-bottom: 20px; margin-left: 6px" :key="1"></Button>
		<div class="p-grid" :key="2">
			<div class="p-col-12">
				<div id="invoice-content">
					<div class="card invoice-wrapper">
						<div class="p-grid p-nogutter invoice-header">
							<div class="p-col-6">
								<h1 class="title">INVOICE</h1>
							</div>
							<div class="p-col-6 logo-adress">
								<img src="assets/layout/images/extensions/prime-logo.svg" alt="ultima-layout"/>
								<p>9137 3rd Lane California City, CA 93504.</p>
							</div>
						</div>
						<div class="card invoice-table billto-table">
							<div class="p-grid p-nogutter table-header">
								<div class="p-col-4">
									<h2 class="header">BILL TO </h2>
								</div>
								<div class="p-col-3">
									<h2 class="header">DATE </h2>
								</div>
								<div class="p-col-3">
									<h2 class="header">INVOICE NO </h2>
								</div>
								<div class="p-col-2">
									<h2 class="header">NOTES </h2>
								</div>
							</div>
							<div class="p-grid p-nogutter table-content-row">
								<div class="p-col-4">
									<p>TOYOKSU SYSCOM CORPORATION 11-27, MEIEKI 4-CHROME
										NAKAMURA-KU, NAGOYA 450-0002 JAPAN </p>
								</div>
								<div class="p-col-3">
									<p>30/08/2019 </p>
								</div>
								<div class="p-col-3">
									<p>A/3100 </p>
								</div>
								<div class="p-col-2">
									<p>N/A </p>
								</div>
							</div>
						</div>
						<div class="card invoice-table products-table">
							<div class="p-grid p-nogutter table-header">
								<div class="p-col-4  row-title">
									<h2 class="header">DESCRIPTION </h2>
								</div>
								<div class="p-col-2">
									<h2 class="header">QUANTITY </h2>
								</div>
								<div class="p-col-3">
									<h2 class="header">UNIT PRICE </h2>
								</div>
								<div class="p-col-3">
									<h2 class="header">LINE TOTAL </h2>
								</div>
							</div>
							<div class="p-grid p-nogutter table-content-row">
								<div class="p-col-4 row-title">
									<p>1 Year PrimeFaces Elite Subscription </p>
								</div>
								<div class="p-col-2">
									<p>4 </p>
								</div>
								<div class="p-col-3">
									<p>$99.00</p>
								</div>
								<div class="p-col-3 total">
									<p>$396.00 </p>
								</div>
								<div class="p-col-4 row-title">
									<p>PrimeFaces Ultima Extended License </p>
								</div>
								<div class="p-col-2">
									<p>1</p>
								</div>
								<div class="p-col-3">
									<p>$790.00</p>
								</div>
								<div class="p-col-3 total">
									<p>$790.00 </p>
								</div>
								<div class="p-col-4 row-title">
									<p>PrimeFaces Prestige Basic License </p>
								</div>
								<div class="p-col-2">
									<p>2</p>
								</div>
								<div class="p-col-3">
									<p>$59.00</p>
								</div>
								<div class="p-col-3 total">
									<p>$118.0</p>
								</div>
							</div>
						</div>
						<div class="p-grid p-nogutter">
							<div class="p-col-12 p-md-12 p-lg-6 table-g-6">
								<div class="p-grid">
									<div class="card invoice-table bank-table">
										<div class="p-grid p-nogutter table-header">
											<div class="p-col-12 row-title ">
												<h2 class="header">BANK </h2>
											</div>
										</div>
										<div class="p-grid p-nogutter table-content-row">
											<div class="p-col-4 row-title">
												<h2>BANK</h2>
											</div>
											<div class="p-col-8">
												<p>BestBank </p>
											</div>
											<div class="p-col-4 row-title">
												<h2>ACCOUNT BENEFICIARY</h2>
											</div>
											<div class="p-col-8">
												<p>Edward Williams</p>
											</div>
											<div class="p-col-4 row-title">
												<h2>SWIFT</h2>
											</div>
											<div class="p-col-8">
												<p>PJNWBXND</p>
											</div>
											<div class="p-col-2 row-title">
												<h2>IBAN</h2>
											</div>
											<div class="p-col-10">
												<p>GB04420235692263866724650931</p>
											</div>
										</div>

									</div>
								</div>
							</div>
							<div class="p-col-12 p-md-12 p-lg-6 table-g-6">
								<div class="p-grid">
									<div class="card invoice-table">
										<div class="p-grid p-nogutter table-header">
											<div class="p-col-12 row-title">
												<h2 class="header">TOTAL </h2>
											</div>
										</div>
										<div class="p-grid p-nogutter table-content-row">
											<div class="p-col-9">
												<h2>SUBTOTAL</h2>
											</div>
											<div class="p-col-3 total">
												<p>$1304.00</p>
											</div>
											<div class="p-col-9">
												<h2>VAT</h2>
											</div>
											<div class="p-col-3 total">
												<p>$234.72 </p>
											</div>
											<div class="p-col-9">
												<h2>TOTAL</h2>
											</div>
											<div class="p-col-3 total">
												<p>$1538.72 </p>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</template>

<script>
export default {
	methods: {
		printClick() {
			window.print();
		}
	}
}
</script>

<style scoped>

</style>
